---
name: effective-java
description: Apply distilled Effective Java principles when writing, reviewing, refactoring, or explaining Java code. Use for Java API design, constructors, static factories, builders, dependency injection, immutability, equals/hashCode/toString/clone/Comparable, class and interface design, generics, enums, annotations, lambdas, streams, method contracts, defensive copying, exceptions, concurrency, serialization, or any request mentioning Effective Java, Java best practices, Java code review, or Java refactoring.
---
...
2. Load `references/review-checklist.md` for reviews/refactors or when code is provided.
...
6. If editing code, make the smallest change that preserves existing behavior and public compatibility unless the user asks for an API redesign.
...
- Use enums instead of integer/string constants; use `EnumSet` and `EnumMap` for enum-keyed collections.
...
- Validate parameters at boundaries; return empty collections/arrays instead of `null` for multi-value returns.
...
For refactoring, explain the chosen item families, apply a focused patch, then list behavior/compatibility assumptions and validation performed.
...
- Do not add frameworks or large abstractions when a Java language/library feature solves the problem.
