...
58. Prefer enhanced `for` loops when indexes/iterators are not needed.
59. Know and use the standard libraries.
60. Use `BigDecimal`, `int`, or `long` for exact monetary calculations; avoid float/double for exact answers.
