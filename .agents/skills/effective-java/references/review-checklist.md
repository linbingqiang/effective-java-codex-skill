...
- Static factories: use when a name clarifies intent, instances are controlled/cached, return type can be an interface/subtype, or construction can hide complexity.
...
- Resource cleanup: use try-with-resources for `AutoCloseable`; avoid finalizers/cleaners except as last-resort safety nets.
...
- Visibility: make classes, constructors, methods, fields, and nested types as private/package-private as possible.
...
- Inheritance: prefer composition; if inheritable, document override hooks and self-use, or prohibit with `final`/private constructors.
...
- Prefer generic methods/classes over casts at call sites.
...
- Replace integer/string constants with enums when the value set is known.
...
- Return empty collections/arrays for no results; avoid `null` multi-value returns.
...
- Use exceptions for exceptional conditions, not normal loop/control flow.
...
- Consider serialization proxy for classes with invariants.
